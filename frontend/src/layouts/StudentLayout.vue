...
const { items: sidebarItems } = useSidebar('/dashboard')
...
const items = computed(() => {
  return sidebarItems.value.filter((item: any) => {
    if ((item.to === '/dashboard/applications' || item.to?.startsWith('/dashboard/applications')) && !isModuleEnabled('applications')) return false
    if (item.to === '/dashboard/messages') return false
    return true
  })
})
